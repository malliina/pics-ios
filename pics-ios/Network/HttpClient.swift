...
class HttpClient {
  ...
  func postJSON<T: Encodable>(_ url: URL, headers: [String: String] = [:], payload: T) async throws
    -> HttpResponse
  {
  ...
  func postData(_ url: URL, headers: [String: String] = [:], payload: Data) async throws
    -> HttpResponse
  {
    let req = buildRequestWithBody(
      url: url, httpMethod: HttpClient.post, headers: headers, body: payload)
    return try await executeHttp(req)
  }

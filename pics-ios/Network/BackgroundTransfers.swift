...
//
class BackgroundTransfers: NSObject, URLSessionDownloadDelegate, URLSessionTaskDelegate,
  URLSessionDelegate
{
  ...
  func urlSession(
      ...
      let written = StorageSize.fromBytes(totalBytesWritten)
    {
      ...
      if taskOpt == nil {
        ...
        log.info(
          "Unable to parse bytes of download progress: \(bytesWritten), \(totalBytesWritten)")
      }
    }
  }
    ...
    if deleteOnComplete {
      ...
      saveUpload(
        task: UploadTask(
          ...
          folder: file.deletingLastPathComponent().deletingLastPathComponent().lastPathComponent,
  ...
  private func removeUploadedAndUploadNext(id: Int, deleteFile: Bool) async {
    if let removed = PicsSettings.shared.removeUpload(id: id) {
      ...
      log.error(
        "Unable to find upload task with ID \(id). Searched \(PicsSettings.shared.uploads.count) tasks."
      )
    }
  }

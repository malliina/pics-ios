...
class PicsLibrary {
  ...
  func load(from: Int, limit: Int) async throws -> [PicMeta] {
    let response = try await http.picsGetParsed(
      "/pics?offset=\(from)&limit=\(limit)", PicsResponse.self)
    return response.pics
  }

  func save(picture: Data, clientKey: ClientKey) async throws -> PicMeta {
    let response = try await http.picsPostParsed(
      "/pics", data: picture, clientKey: clientKey, PicResponse.self)
    return response.pic
  }
  ...
  func syncOffline(for user: Username) {
    ...
    synchronized {
      ...
      do {
        ...
        let now = Date()
        LocalPics.listFiles(at: uploadingDir).filter { now.timeIntervalSince($0.created) > oneDay }
          .forEach { url in
            let dest = stagingDir.appendingPathComponent(url.lastPathComponent)
            do {
              try FileManager.default.moveItem(at: url, to: dest)
              self.log.info(
                "Moved \(url) to \(dest) because it had not been uploaded in a reasonable amount of time."
              )
            } catch let err {
      ...
      files.headOption().map { file in
        do {
          ...
          self.log.info(
            "Syncing \(uploadingUrl) for '\(user)' taken at '\(file.created)'. In total \(files.count) files awaiting upload."
          )
          self.uploadPic(
            picture: uploadingUrl,
            clientKey: LocalPics.shared.extractKey(name: file.lastPathComponent)
              ?? ClientKey.random(), deleteOnComplete: true)
        } catch let err {
  ...
  func uploadPic(picture: URL, clientKey: ClientKey, deleteOnComplete: Bool = false) {
    ...
    BackgroundTransfers.uploader.upload(
      url, headers: headers, file: picture, deleteOnComplete: deleteOnComplete)
  }
}

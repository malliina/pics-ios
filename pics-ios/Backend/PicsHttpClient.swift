...
class PicsHttpClient: HttpClient {
  ...
  convenience init(accessToken: AWSCognitoIdentityUserSessionToken?) {
    if let accessToken = accessToken {
      self.init(
        baseURL: EnvConf.shared.baseUrl,
        authValue: PicsHttpClient.authValueFor(forToken: accessToken))
    } else {
  ...
  func picsPostParsed<T: Decodable>(
    _ resource: String, data: Data, clientKey: ClientKey, _ to: T.Type
  ) async throws -> T {
  ...
  {
    let url = urlFor(resource: resource)
    return try await statusChecked(
      response: self.postData(url, headers: headersFor(clientKey: clientKey), payload: payload))
  }
  ...
  func modify(key: ClientKey, access: AccessValue) async throws -> HttpResponse {
    let req = try build(
      path: "/pics/\(key)", method: HttpClient.post, body: ModifyBody(access: access))

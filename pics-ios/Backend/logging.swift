...
class LogsTokenSource: TokenSource {
  ...
  func fetchToken() async throws -> AccessToken {
    return try await client.postJSON(fullUrl(to: "/sources/token"), headers: LogsHttpClient.headers, payload: LogTokenRequest(app: "pics-ios")).to(LogTokenResponse.self).token
  }
...
class LogsHttpClient {
  ...
  func send(logs: [LogEvent], attempt: Int = 1) async throws -> LogsSentResponse {
    ...
    let response = try await client.postJSON(url, headers: allHeaders, payload: LogEvents(events: logs))
    if response.isStatusOK {
      ...
      if attempt == 1 && response.isTokenExpired {
        ...
        let error = ErrorMessage("Failed to handle \(response.statusCode) response from \(url) after \(attempt) attempts.")

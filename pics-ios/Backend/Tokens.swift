...
class Tokens {
  ...
  func retrieveUserInfoAsync(cancellationToken: AWSCancellationTokenSource? = nil) async throws
    -> UserInfo
  {
    return try await withCheckedThrowingContinuation { cont in
      guard let user = poolOpt?.currentUser() else {
        return cont.resume(throwing: AppError.simple("Unknown user."))
      }
      user.getSession().continueWith(
        block: { (task) in
          if let error = task.error as NSError? {
            let appError =
              error.localizedDescription == AppError.noInternetMessage
              ? AppError.noInternet(error) : AppError.tokenError(error)

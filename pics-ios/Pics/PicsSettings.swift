...
class PicsSettings {
  ...
  func save(pics: [PicMeta], for user: Username) -> ErrorMessage? {
    let refs = pics.map { pic in
      PicRef(filename: pic.url.lastPathComponent, access: pic.access, added: pic.added)
    }
  ...
  func localPictures(for user: Username) -> [PicMeta] {
    let metas =
      prefs.load(key(for: user), PicRefs.self)?.pics.filter({ ref in

...
class LocalPics {
  ...
  init() {
    let dirString =
      NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0] + "/pics"
  ...
  func baseDirectory(folder: String, sub: String) -> URL {
    dir.appendingPathComponent(folder, isDirectory: true).appendingPathComponent(
      sub, isDirectory: true)
  }

  func createdFor(url: URL) -> Date {
    if let values = try? url.resourceValues(forKeys: [URLResourceKey.creationDateKey]),
  ...
  func extractKey(name: String) -> ClientKey? {
    if name.startsWith(localPrefix) && name.endsWith(jpgExt)
      && name.count > (localPrefix.count + jpgExt.count)
    {

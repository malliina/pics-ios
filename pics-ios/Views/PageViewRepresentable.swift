...
struct PicPageView<T>: View where T: PicsVMLike {
  ...
  var body: some View {
    PageViewRepresentable(
      viewModel: viewModel, startIndex: startIndex, isPrivate: isPrivate, active: $active
    )
...
struct PageViewRepresentable<T>: UIViewControllerRepresentable where T: PicsVMLike {
  ...
  func makeUIViewController(context: Context) -> UIPageViewController {
    ...
    let swipeRecognizer = UISwipeGestureRecognizer(
      target: context.coordinator, action: #selector(context.coordinator.onSwipeUp(_:)))
  ...
  class Coordinator: NSObject, UIPageViewControllerDelegate, UIPageViewControllerDataSource {
    ...
    func pageViewController(
      ...
      if completed {
        ...
        guard
          let hosting = pageViewController.viewControllers?.first as? UIHostingController<PicView>
        else {

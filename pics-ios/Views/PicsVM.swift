...
extension PicsVM: PicsDelegate {
  ...
  private func contains(pic: PicMeta) -> Bool {
    self.pics.contains(where: { p -> Bool in
      (pic.clientKey != nil && p.clientKey == pic.clientKey) || p.key == pic.key
    })
  }
}
...
class PicsVM: PicsVMLike {
  ...
  init(userChanged: @escaping (Username?) -> Void) {
    ...
    let cognitoDelegate = CognitoDelegate(
      onShowLogin: {
  ...
  private func merge(onlinePics: [PicMeta], more: Bool) async {
    ...
    log.info(
      "Replacing gallery with \(onlinePics.count) pics. Added \(added.count) and removed \(removed.count) pics."
    )
      ...
      do {
        if let user = user {
          ...
          let userInfo = try await Tokens.shared.retrieveUserInfoAsync(
            cancellationToken: authCancellation)
  ...
  private func removeLocally(keys: [ClientKey]) async {
    animatePics = true
    await savePics(
      newPics: self.pics.filter { pic in
